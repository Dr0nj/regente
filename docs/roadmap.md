...
- **Núcleo / enterprise clássico** — GitOps, daily imutável, dependências com condições, Force Order, executores.
...
- Dependência com CVE. O `.github/dependabot.yml` (mensal, agrupado) transforma isso em uma
  ...
  — **ligado de verdade em 2026-07-30** (`vulnerability-alerts` + `automated-security-fixes`
  estavam DESLIGADOS quando esta seção foi escrita, ver changelog).
- **Falha de segurança no código PRÓPRIO** que exponha credencial, fure autenticação ou corrompa
  a trilha de auditoria. Acrescentado em 2026-07-30, quando o piso original (só "CVE em
  dependência") não cobriu o `middleware.RealIP` — um GHSA que chegou POR uma dependência mas
  cuja correção era no nosso código. Mesma lógica do CVE: não é feature, é dívida que só piora,
  e o achado é pontual. ⚠️ **Isto não é porta pra UX**: mensagem confusa, dev-mode que engana e
  "ficou feio" continuam congelados — o critério é exposição de credencial / autenticação /
  auditoria, não desconforto.
...
- [x] **Z (artefatos)** — Case study técnico + post LinkedIn — **ENTREGUES (2026-07-13):**
  ...
  enterprise clássico → escala 1M validada → lições) e [`docs/linkedin-post.md`](linkedin-post.md)
...
- [ ] **AI-1 — JobType `AI_AGENT`: análise por LLM local, isolada, auditável — SEM tools.**
  ...
  **Posicionamento (o porquê de mercado).** O público-alvo do Regente é o mundo
  enterprise clássico: bancos, seguradoras, utilities — ambientes **regulados** onde sysout,
...
✅ Paridade enterprise clássica (calendars · resources · conditions · vars · SLA · forecast)
...
## 📏 ST-1 — Statistics honesta: a EXECUÇÃO virou registro próprio *(2026-08-05)*

> **O achado (auditoria pedida pelo usuário):** job de 2 minutos exatos exibia `avg 10min` e
> `max 98min` na aba Statistics. A duração era derivada de `finished_at − started_at` da
> **instance** — e esse par não delimita uma execução: quem o move não é só o job. Um **Set OK**
> num WAITING que já tentou (o retry agendado zera o `finished_at`) carimba `finished_at=AGORA`
> sobre um `started_at` de horas atrás, e aquilo entrava na estatística como "esta run durou 98
> minutos". No mesmo balaio: retry e cyclic sobrescrevem os timestamps a cada tentativa/volta (as
> anteriores somem da amostra) e um Set OK em quem **nunca rodou** contava como "run". Com o
> watchdog de RUNNING em 15 min, aliás, nenhuma execução real chega a 98 minutos — o número só
> podia vir de fora da execução.

```
✅ instance_runs (schemaV23) — 1 linha por EXECUÇÃO REAL: abre no claim WAITING→RUNNING
   (a mesma transição que carimba started_at), fecha no fim da TENTATIVA (FinishInstance,
   inclusive quando o retry re-arma) e some se o claim é revertido por falta de agente
✅ regra do operador: começou = entrou em RUNNING; ficar em WAIT não conta. Set OK/cancel de
   quem não está rodando não abre nem fecha execução (e não vira "run" na contagem)
✅ Statistics, PerfForecast (p50/p90/próxima) e MTTR do analytics medem sobre instance_runs —
   min/avg/max deixam de somar espera e ação de operador
✅ aba Statistics ganha LAST RUNS: as 10 últimas execuções com início, fim, tempo de execução e
   status (+ exit code no NOTOK, marca de tentativa no retry) — paridade com a lista do
   orquestrador enterprise clássico; "This run" passa a mostrar a duração da execução
✅ card do Monitoring lê "início–fim" quando a run terminou (só o início enquanto roda), e o
   fim viaja NO evento de término — a caixinha não espera o próximo GET cheio
✅ backfill na migration (uma execução por instance que chegou a rodar) — histórico existente
   não desaparece da aba; archiveGC leva as execuções junto com a daily arquivada
✅ 6 testes novos (runs_test.go), com a regressão do caso auditado, + Recent/ordem em jobstats
```
...
✅ ACTIVE JOBS COLAPSÁVEL (2026-08-07) — o painel encolhe para um TRILHO de 44px no canto e volta
   com um clique. `useResizablePanel` ganhou `railWidth` (opcional; ausente = painel não colapsa) e
   passou a devolver `collapsed`/`toggleCollapsed`, com o estado persistido em
   `<storageKey>.collapsed`. A largura expandida NÃO é tocada pelo colapso — reabrir devolve os
   px que o operador tinha arrastado. O trilho não é decoração: mantém o total do dia (abreviado
   PRA BAIXO, `fmtCompact` — inflar contagem visível seria mentir sobre o dia) e os 4 status, e
   clicar num status reabre o painel JÁ FILTRADO. O trilho continua sendo o overlay
   `data-canvas-inset="left"` (só que com 44px), então a centralização do canvas segue honesta sem
   ninguém avisar a câmera — e a câmera NÃO se move sozinha ao colapsar (quem move a câmera é o
   usuário). **GOTCHA da virtualização:** no trilho o scroller nem existe; sem `railed` nas deps, o
   efeito do `ResizeObserver` rodaria uma única vez (montagem já colapsada = ref nula) e a lista
   reabriria presa no `viewH` default, virtualizando por uma altura inventada — o efeito de medida
   E o de paginação (`ensureRange`) passaram a depender do colapso. Validado ao vivo nos 4 caminhos:
   colapsar/expandir, montar já colapsado + expandir (scroller re-medido em 450px, não nos 600 do
   default), largura custom 520px preservada no ciclo, e clique no status reabrindo filtrado.
✅ Diálogos padronizados (✕ + Cancelar/Salvar via classes compartilhadas); Control Panel (ex enterprise clássico)
...
✅ Selo ✓/✗ da condição nas edges do grafo tematizado (2026-07-29) — era hex chumbado em
   `edgeStyleForState`, virava quadrado quase preto sobre o canvas dos 4 temas claros. Agora sai de
   `--v2-edge-{ok,blocked,idle}[-label|-bg]`, DERIVADOS por `color-mix()` dos `--v2-status-*` + fundo
   (tema novo herda o selo certo sem repetir hex); os claros reabrem só esses tokens e invertem o selo
...
✅ Aba Schedule do job redesenhada (2026-06-29) — `ScheduleEditor`: (1) SAÍRAM "dia útil" e "regra avançada"
   ...
   clássico "View Scheduling" (12 mini-meses, seletor de ano) onde os dias DESTACADOS são EXATAMENTE os que o
...
> **Paridade enterprise clássica:** **Output = sysout da EXECUÇÃO** (o que o processo escreveu,
...
## 🌐 Idioma — produto 100% em inglês (i18n, 2026-07-24)

> **Regra:** tudo que o USUÁRIO lê é inglês; tudo que o DESENVOLVEDOR lê segue em pt-BR.
> Comentário de código, mensagem de commit e docs do repo **não** mudam.

```
✅ I18N-1 — SPA: rótulos, abas, validações, toasts, popovers de ajuda ("?"), editor
   de Schedule (Mon–Sun/Jan–Dec, From/To, locale en-GB), temas (nomes/descrições) e o
   guia do dialeto YAML do modo Code (`code-schema.ts`, ~650 linhas reescritas).
✅ I18N-2 — Server: erros de validação e de política que sobem pra UI, `typeschema.go`
   (summary + desc de TODOS os jobTypes), Explain (blockers/summaries), RCA, bulk,
   alertas do control plane (selfmon), rótulos de escopo de condição (static/daily) e
   o catálogo de ferramentas MCP.
✅ I18N-3 — CLI e agente: help das flags, usage e saída de `regente`/`regente-server`;
   output dos jobs do agente (mft/file-watch/timeouts/truncamento); CLIs de apoio
   (`importctm` com relatório e `# TODO-import`, `docsite`, `seed`).
✅ I18N-4 — Contrato público: `openapi.yaml` reescrito em inglês (info/tags/summaries/
   descriptions/schemas) + viewer `/api-docs` (`apidocs.html`, `lang=en`).
✅ I18N-5 — Scripts de instalação e operação (o que o OPERADOR vê): one-liner
   `install.sh`, installers de server/agente (Linux/macOS/Windows), `regente-configure`,
   backup/restore/rolling-upgrade/chaos-ha, enable-tls, sandbox-agent, host-demo, verify.

NÃO traduzidos DE PROPÓSITO:
  · termos canônicos do domínio — Schedule · On/Do · OK/NOTOK · ODAT · carry-over ·
    Force Order · Run Now · Set OK · Hold/Release · WAIT_AGENT/WAIT_COND/…
  · identificadores de MÁQUINA — ids de tema (viram seletor CSS `data-theme` +
    localStorage), motivos de carry, `GateKind`, nomes de jobType, tokens de condição.
  · sinônimos PT do parser de NL-query (`api/query.go`): são tokens de ENTRADA —
    a resposta já sai em inglês.

✅ I18N-6 — DOCUMENTAÇÃO PÚBLICA em inglês + empacotada (2026-07-27). READMEs
   (raiz, server, agent, app, deploy/, deploy/vps/, deploy/demo/, server/deploy/,
   cmd/regente, cmd/importctm) e os docs publicados; `operacao.md` →
   `operations.md` e `arquitetura-futuro.md` → `architecture-future.md` (refs
   reescritas no código, docs e workflows). O README raiz foi AUDITADO, não só
   traduzido — ver o changelog. `roadmap.md` (este arquivo) fica em pt-BR e FORA
   do site: é documento de trabalho nosso (`skipDoc` no docsite, junto de
   linkedin-post, case-study PT e os planos internos).
✅ I18N-7 — o site abre sem subir nada (2026-07-27). `docs/site/` saiu do
   .gitignore e é build VERSIONADO (clona/baixa → abre o index.html), com passo
   no CI que regenera e falha se defasar; o docsite emite também a referência de
   API (`api.html` = o viewer do /api-docs com a spec INJETADA, porque fetch é
   bloqueado em file://; Try-it e campo de token somem no modo estático).
   Publicado no GitHub Pages: **https://dr0nj.github.io/regente/**.
```
...
✅ Operação   → OpenTelemetry ✔ (opt-in OTLP) · **zero-downtime VALIDADO** (rolling-upgrade.sh: novo sobe
                 ...
                 branch+DB+env_label; `regente_env_info` no /metrics) · **quotas** (F15 + rebuild HA) — docs/operations.md
...
## 🎛️ Aprofundamento enterprise *(trilha fechada — 2026-07-06)*
...
✅ Actions / On-Do (On/Do clássico) — motor nas 4 dimensões (result/exit/attempt/runtime) + 4 ações
   (notify/set-condition/run-job/set-ok), idempotência por ledger (migration v7). UI: aba On/Do no drawer.
   `on: exit` (2026-07-23) casa pelo EXIT CODE — lista/faixa/comparação ("1,2,3"·"1-4"·">=8"·"!=0",
   COMPSTAT do modelo clássico); com `set-ok` = "trate estes códigos como sucesso".
...
## 🌟 Diferenciais além do modelo clássico *(trilha fechada — leva 1: 2026-07-03 · leva 2 D-1..D-15: 2026-07-08)*

> Onde o Regente **passa** o orquestrador clássico. Todos read-only-aditivos ou opt-in — nenhum toca o
...
✅ D-2b Hold geral + Delete de instance (2026-07-16) — Hold individual/bulk também vale pra qualquer status
        ...
        + ação bulk "delete" (o "Delete job" clássico): remove a ordem da tela/state store, SÓ com o job em HOLD
...
✅ ADV-1 — Schema DEDICADO por jobType (fonte única declarativa)
   • domain/typeschema.go: registry com os 14 tipos (COMMAND/SCRIPT/SSH/HTTP/DATABASE/FILE_WATCH/
     FILE_TRANSFER/LAMBDA/BATCH/GLUE/STEP_FUNCTION/WASM/K8S/GCP_RUN) — campos, obrigatórios,
     ...
     Campo DESCONHECIDO = erro citando o typo e os aceitos.
     [WAIT, CHOICE e PARALLEL removidos em 2026-07-22 — os 3 "gate types" agentless nunca
      tiveram executor real; ver §Changelog.]
...
✅ ADV-4 — MFT: jobType FILE_TRANSFER nativo (alias MFT), executado pelo agente
   ...
   • Semântica do MFT clássico: glob (*.csv) na ORIGEM local/sftp (múltiplos arquivos exigem
...
### 🧯 V-AUDIT — a instalação vista por quem NUNCA viu o Regente (2026-07-27)

> Auditoria pedida pelo usuário ("com as info que estão lá eu consigo instalar de forma
> fácil sendo leigo o Regente em um servidor?"). A resposta era **não**: o caminho "fácil"
> (release) dava 404 em TODOS os arquivos e o de código-fonte entregava server sem UI.
> O workspace GitOps apontava pro lab do autor — **cada instalação tem o SEU repo**.

```
✅ VA-1 · Release não sai mais pela metade — release.yml reestruturado: os 5 alvos viram
       artifacts e um job `publish` ÚNICO monta tudo (antes 5 jobs chamavam
       action-gh-release em paralelo contra a MESMA release → a v0.1.0 saiu com 6 assets,
       sem install.sh, sem os installers de agente e sem os bundles .tar.gz). Passo
       `Verify the release payload` FALHA antes de publicar se faltar qualquer arquivo
       que o README promete, gera SHA256SUMS, e workflow_dispatch republica uma tag.
✅ VA-2 · Install do código-fonte voltou a instalar a UI — SPA_SRC só procurava
       $HERE/../app/dist (layout do BUNDLE); no checkout o app está em ../../app/dist,
       então caía silenciosamente em API-only. Agora procura nos dois + aceita SPA_DIR.
✅ VA-3 · Zero default do lab — `-git-source`/`-github-repo` nasciam apontando pro
       Dr0nj/regente-workspace (404 pra terceiros) no BINÁRIO, no server.env.example, no
       configure.sh e no host-demo.ps1. Default agora é vazio = offline; o repo é
       perguntado, validado (owner/name) e é sempre O DO OPERADOR.
✅ VA-4 · Repo VAZIO é o caminho normal — EnsureClone trata ErrEmptyRemoteRepository
       fazendo o bootstrap (init + definitions/.gitkeep + .gitignore + README + commit +
       push): "crie um repositório vazio no GitHub" virou instrução suficiente. Também
       ADOTA um workspace que já tem conteúdo quando o remote está vazio (migração
       offline→GitOps) e segue a branch default do repo quando a pedida não existe
       (repo com `master`), em vez de morrer com "reference not found".
✅ VA-5 · Boot deixou de ser fatal — repo/PAT errado dava log.Fatalf e, com
       Restart=always, CRASH-LOOP: o operador ficava sem UI justamente onde arrumaria a
       config. Agora sobe, expõe o motivo em /api/git/status (`error`) e no badge
       (⚠ not connected), e um retry com backoff (15s→5min) reconecta sozinho. Só a
       daily é pulada até lá. `-github-repo` inválido também virou aviso, não fatal.
✅ VA-6 · Retry que REALMENTE recupera (achado rodando o server de verdade): o próprio
       server cria <workspace>/definitions no boot, e o guard de "not empty" passava a
       barrar todas as tentativas seguintes — recuperar exigia rm -rf manual. IsEmpty
       passou a ignorar diretórios vazios.
✅ VA-7 · UX de instalação — install-linux.sh faz health check e imprime os próximos
       passos NA ORDEM (configure → restart → FIREWALL → abrir a URL, com o IP real);
       installers de agente normalizam a URL (http://host:8080 → ws://host:8080/ws/agent),
       guardam o token em EnvironmentFile 0600 (era visível no `ps`) e conferem se o
       serviço ficou de pé; configure.sh mostra se o GitOps conectou de verdade.
✅ VA-8 · README com o que faltava pro leigo: pré-requisitos (máquina/porta/repo+PAT),
       "Before you start", primeiro job em 4 passos, tabela de troubleshooting e a
       correção de que só server/deploy e agent/deploy têm install-windows.ps1.
✅ VA-9 · VALIDAÇÃO em VPS de mentira (container systemd, Ubuntu 22.04) rodando o
       release PUBLICADO — e ela achou 2 bugs que a suíte não pegava:
       (a) clone que falha no TRANSPORTE (repo/PAT errado sobre HTTPS) deixa um .git
           PARCIAL para trás → todo EnsureClone seguinte cai no ramo IsRepo e a
           instalação NUNCA mais se recupera, nem com a config corrigida (só rm -rf).
           Falha de clone agora limpa o workspace (só se chega ali com ele vazio).
       (b) com esse .git meio-inicializado (zero commits) + remote vazio, o "publish"
           empurrava NADA — go-git não erra ao dar push de branch inexistente — e o
           fetch seguinte reclamava "empty" para sempre. Agora: sem commit na branch,
...
> one-liner instala e o health responde · repo inexistente → serviço **active, NRestarts=0**
> (o binário da v0.1.0, mesma config, sai com exit 1 = crash-loop) · repo VAZIO → bootstrap
> + push · `docker restart` (reboot) → serviço sobe sozinho, workspace/DB/UI intactos ·
...
✅ VL-9 · UPGRADE de verdade — reinstalar por cima é o caminho de atualização que o README
       manda usar, e ele NÃO atualizava: `systemctl enable --now` só INICIA unit parada; numa
       unit já ativa o start é no-op, então o binário novo ia pro disco e o processo ANTIGO
       seguia rodando. O operador reinstalava, via tudo "ok" e nada mudava. Agora o installer
       detecta o serviço vivo ANTES de tocar em nada (guarda o MainPID), **restarta**, imprime
       `UPGRADED in place` + `Process replaced: PID a -> b` (a prova de que a versão nova está
       no ar) e um resumo do que foi preservado × substituído — em vez dos "próximos passos"
       de instalação nova, que viravam ruído. Se o PID não mudar, AVISA e manda restartar.
       README ganhou a seção **Upgrading a running installation** (tabela mantido × trocado,
       migrações no boot, agentes são binários separados, zero-downtime só com N nós+Postgres).
       Smoke reinstala por cima e prova: upgrade reconhecido · PID trocado · health 200 ·
       server.env intacto · banco preservado · deploy/vps ainda instalado.
✅ VL-10 · VERSÃO visível no rodapé da UI (pedido do usuário: "uma versão no Regente na UI,
       refletindo o release, pra gente saber se realmente atualizou") — fecha o par com o
       VL-9: o instalador prova a troca no MOMENTO do upgrade, isto prova DEPOIS, a qualquer
       hora, sem SSH. `var version` no `main.go` injetada por `-ldflags "-X main.version=<tag>"`
       (release.yml usa `needs.version.outputs.tag`; build do fonte fica **"dev"**, que é
       honesto — não há release por trás). Três superfícies: log de boot, **`regente-server
       -version`** (responde antes de abrir banco/porta, então roda numa caixa com o serviço
       de pé) e **`GET /api/version`**, que é o que o rodapé lê. O endpoint fica ATRÁS da
       autenticação de propósito: dizer a versão exata a um anônimo entrega a lista de CVEs
       aplicáveis junto, e o `/health` (público) não precisa disso. **GOTCHA que só apareceu
       no browser:** buscar no mount devolvia VAZIO — o V2Preview monta com a tela de login
       por cima e o `/api` responde 401 a quem ainda não entrou; o efeito passou a depender
       de um booleano derivado do estado de auth (`me` troca de identidade a cada refetch e
       re-dispararia o efeito à toa). Validado AO VIVO: server local com
       `-X main.version=v9.9.9-local` → rodapé em `build v9.9.9-local`. Smoke builda com
       `smoke-test` e prova a cadeia inteira (ldflags → binário → API → `-version` batendo
       com a API).
```
...
|----|--------|---------|
| ✅ | ~~**Menu FORCE: jobs escondidos dentro da folder + filtro**~~ | **Feito (2026-08-24, no mesmo dia do Order Folder, pedido do usuário.)** O menu listava as definitions numa lista PLANA — com a folder de teste virou um scroll de 200 linhas onde "fica mt difícil achar um job". Agora é uma **árvore**: folder colapsada por default, jobs só aparecem no clique. **A linha da folder tem duas ações e nenhuma escondida em hover**: o corpo abre/fecha, o chip **ORDER** à direita ordena a folder inteira (`stopPropagation`) — ordenar continua a 1 clique, sem virar 2. **Filtro por texto** junto (só aparece com >12 jobs, senão é ruído): colapsar sozinho não resolve, porque a folder ABERTA ainda são 200 linhas. Casar com o nome do JOB filtra dentro da folder e mostra `1/200` no contador; casar com o nome da FOLDER traz a folder inteira (procurar pela folder é procurar por tudo que está nela); o que sobra vem **aberto** — deixar o resultado da busca atrás de mais um clique é o oposto de procurar. Jobs sem `team` caem num grupo "— no folder —" que não ganha o chip ORDER. **Validado com os 200 jobs:** menu fechado = 4 linhas · abrir a folder = 200 jobs · fechar = 0 · filtro `0177` = `1/200` com a folder auto-aberta · filtro `ACTIONS` casando no nome da folder = folder inteira · filtro sem match = "No job matches this filter" · chip ORDER disparando da linha COLAPSADA sem expandir. |
| ✅ | ~~**Order Folder — ordenar a folder INTEIRA na diária ativa**~~ | **Feito (2026-08-24, pedido do usuário.)** Faltava o gesto em massa do que já existia por job: colocar uma folder inteira pra rodar HOJE. Caso de uso real — folder cujo calendário não bateu, folder criada depois da daily, folder com `schedule.enabled: false`. **`POST /api/folders/{name}/order`** (writer + write na folder, nome pelo `urlName` porque o chi entrega percent-encoded) → **`Scheduler.OrderFolder`**. **Semântica = a do Order Force individual (`force_mode='order'`)**: bypassa só o AGENDAMENTO — calendário, frequência, `schedule.enabled`. Condições, agente, recursos, janela e Confirm continuam valendo, e é exatamente isso que faz a folder rodar **na ordem das dependências** em vez de disparar 200 chamadas de uma vez. **IDEMPOTENTE POR DIA:** definition que já tem instance na diária ativa é PULADA — a resposta separa `ordered` de `skipped` e a UI mostra os dois ("191 ordered · 9 already there"). Sem isso, dois cliques numa folder de 200 viravam 400 cards. **Escala:** INSERT em lote numa transação por chunk (mesmo caminho da daily, `dailyBatchChunk`) e **UM** `Tick()` no fim em vez de repetir o gate+dispatch por job — a folder de 200 do teste ordenou em **53ms**. **Bug DAY-1 fechado de passagem:** o `ForceOrder` gravava `time.Now().Format("2006-01-02")` — data de CALENDÁRIO no relógio do server —, contrariando o que o próprio `BusinessDate` documenta ("ordem forçada e Order Force pertencem ao dia D"). Com `daily_at=06:00`, um Order Force às 02:00 nascia com o order_date de AMANHÃ: **o card sumia do board sem erro nenhum**. Agora os dois caminhos usam `NowLocal()` + `BusinessDate()`. Coberto por teste que fixa o relógio em 02:00 com daily_at=06:00 e exige order_date do dia anterior nos DOIS (folder e job). **UI:** seção "Order Folder — into the active daily" no topo do menu **FORCE ▾** do Monitoring, com contagem de jobs por folder; fica acima da lista de jobs de propósito (numa folder grande a lista vira um scroll longo e a ação de folder sumiria nele). **Validado na folder de 200:** 200 ordenados do zero em 53ms · 2º clique = 0 ordenados / 200 pulados · folder fora do calendário entrando na diária ativa · folder inexistente = 400. 7 testes novos (5 no scheduler, 2 na API). |
| ✅ | ~~**Folder com 200 jobs: o canvas do Design "não arrasta" e engasga**~~ | **Feito (2026-08-24, achado na instância viva.)** O usuário carregou uma folder de teste com **200 jobs HTTP e 256 dependências aleatórias** e reportou: dá zoom, mas arrastar a tela para o lado não anda — e o que anda, anda lento. **Três causas, nenhuma delas o "peso do grafo" em si.** **(1) Zona morta de 83% da tela.** O pan custom (`useCanvasCamera.attachStage`) recusava iniciar quando o `mousedown` caía em `.react-flow__node` — regra herdada de quando o card era arrastável. Só que **nunca existiu `onNodesChange`** no `<ReactFlow>`: arrastar card nunca moveu nada (posição no Design vem do layout + grade da folder), então a regra só **engolia o gesto**. Com 9 jobs sobra fundo pra pegar; com 200, medido no pane, **os cards cobrem 83% da área visível** — 4 em cada 5 tentativas de arrastar caem no vazio funcional. Agora só o **handle** (bolinha de conexão) reserva o arrasto, e `nodesDraggable` foi desligado por ser peso morto. **(2) Sem culling.** Os **201 nós + 267 arestas** ficavam TODOS no DOM dentro de uma camada de **~15800×2150px**; cada frame de pan obrigava o browser a re-rasterizar o conjunto inteiro. Ligado `onlyRenderVisibleElements` → **5–16 nós e 14–47 arestas** no DOM. É seguro justamente por causa do `initialWidth/Height` que já estava lá (o fix dos cards sumindo): o culling precisa do retângulo ANTES da medição. **(3) Minimap re-renderizando 200 `<rect>` por frame.** O `NavMinimap` assinava `s.transform[0..2]` no corpo do componente pra desenhar UM retângulo de viewport — e como a câmera escreve o viewport a cada `mousemove`, os 200 quadradinhos eram reconciliados junto. Separado em `<ViewportRect>` (único assinante do transform) + `<JobsLayer>` memoizado. **De brinde:** o botão **Organize** existia só no Monitoring — numa folder que vira uma faixa de 16000px, ficar sem "voltar pro enquadre" é armadilha; agora vale nos dois modos. **Validado** varrendo a folder inteira nos dois modos: **0 nós com `visibility:hidden`, 0 arestas sem `path`** (a regressão clássica do ReactFlow v12), pan a partir do corpo do card funcionando e handle ainda reservado pra conexão. **Nota honesta:** a folder ficou com 15800px de largura porque o dagre abre dummy-nodes pras arestas que pulam vários ranks — artefato do DAG aleatório do teste (o rank mais cheio tem 22 jobs); um grafo real de produção é bem mais estreito. |
| ✅ | ~~**3 CVEs abertos: go-git segue symlink no worktree (high+medium) e nanoid em loop infinito (high)**~~ | **Feito (2026-08-23, manutenção — piso.)** O `git push` do fix da DSN devolveu o aviso do GitHub de 3 alertas no default branch; varridos no mesmo dia. **(1) `go-git/v5` 5.19.1 → 5.19.2** (`server/go.mod`): GHSA-hc8v-wwc9-vgxm (**high** — operações de worktree podem SEGUIR SYMLINK) e GHSA-qgq7-7hm3-q39j (medium — nome de referência malicioso modifica arquivo fora do storage de refs). **Relevante de verdade aqui, não teórico:** é exatamente a superfície que o GitOps usa — `git.PlainClone` + `repo.Worktree()` em `internal/storage/git.go` (e no `cmd/regente/promote.go`) rodando contra um repositório REMOTO, como o serviço, com o workspace em `/var/lib/regente`. Exige controle do repo de workspace (PAT comprometido, colaborador hostil) — não é acesso anônimo —, mas o efeito seria escrita fora do workspace. O bump arrastou junto `x/crypto` 0.52.0→0.53.0, `x/net` 0.55.0→0.56.0, `x/sync`, `x/sys` e `x/text`, todos transitivos do go-git. O `agent` não usa go-git, ficou de fora. **(2) `nanoid` 3.3.16 → 3.3.18** (`app/package-lock.json`): GHSA-2v37-7h3g-55p8, gerador custom entra em loop infinito com `size` zero. **Impacto real no Regente ≈ nulo** — é transitivo de build (`vite` → `postcss` → `nanoid`), não vai pro bundle servido, e o bug pede um gerador custom que o projeto não tem; o que se fecha aqui é a contagem de alertas, e vale dizer isso em vez de inflar a gravidade. **Correção de LOCKFILE, 3 linhas, sem `overrides`:** o `postcss` já pede `^3.3.x`, então `npm audit fix` resolveu dentro do range e o `package.json` não mudou — deliberadamente diferente do episódio do `brace-expansion` (2026-07-30), em que `overrides` parecia resolver e quebrava o lint. **Validação:** `npm audit` em **0 vulnerabilities**; `go build`/`go vet`/`staticcheck`/suíte inteira do server verdes; **cross-compile `GOOS=linux`**; `tsc -b`, `eslint` e `vite build` limpos. |
| ✅ | ~~**O log de boot imprimia a DSN do Postgres — senha em claro**~~ | **Feito (2026-08-23, manutenção — piso.)** A linha de boot (`server/main.go:604`) imprimia `*dbPath` cru. A flag `-db` guarda **a mesma variável** pros dois backends: com `-db-driver=sqlite` ela é o path do arquivo (inofensivo), com `postgres` é a connection string inteira — `postgres://regente:<senha>@host:5432/db`. Achado numa instalação REAL rodando em Kubernetes, onde a senha vinha de um Secret: **guardar no Secret não adianta se o processo a imprime no boot**. **Por que é sério e não cosmético:** a senha sai do domínio dos segredos e entra no domínio dos logs, que tem política de acesso muito mais frouxa (`kubectl logs`/`journalctl` é permissão banal de plantão) e ainda é agregado, replicado e retido por meses — rotacionar depois exige expurgar histórico de log. **Fix:** `db.RedactDSN` (`server/internal/db/redact.go`), único ponto por onde a variável passa antes de virar texto: forma URL → recorte **textual** do userinfo (não `u.String()`, que re-encoda o `***` como `%2A%2A%2A` e normaliza o resto — o log tem que sair igual ao que o operador configurou, menos a senha); forma keyword/value (`password=x` e `password='x y'`) por regex de uma passada; **path de arquivo sai intacto**, porque mascarar o path só cegaria o diagnóstico. ⚠ Armadilha evitada: a proposta original mandava devolver a string crua quando o parse falhasse — e é exatamente aí que ela TEM senha (o que mais quebra o parse é caractere especial não escapado); o fallback redige do `:` do userinfo até o **último** `@`, mascarando demais em vez de vazar. **Escopo confirmado:** ocorrência única — `dbPath` só aparece em 3 lugares (flag, `db.Open`, o log). O outro `log.Printf` que imprime segredo (`main.go:534`, `REGENTE_TOKEN`) só dispara nos valores de exemplo conhecidos. **Caminho de erro VERIFICADO — não era problema:** havia a suspeita de que `log.Fatalf("db open: %v", err)` vazasse pelo pgx; **não vaza**, e isso foi confirmado RODANDO o binário, não só lendo o fonte — DSN válida contra porta morta imprime `failed to connect to user=regente database=regente`, DSN malformada imprime `cannot parse postgres://regente:xxxxxx@...`, porque o `ParseConfigError.Error()` do pgx v5.10.0 passa por `redactPW` e o `parseURLSettings` desembrulha o `url.Error` justamente pra não devolver a URL crua. **Testes:** 12 casos de forma (URL com/sem senha, sem userinfo, percent-encoded, DSN inválida, keyword/value com e sem aspas, path relativo/absoluto/Windows/com pragmas, vazio) + a invariante que importa (`TestRedactDSNNuncaVazaSenha`: o segredo não sobra na saída em nenhuma forma, e o host continua visível). **Validado ao vivo:** boot real com SQLite → o path continua saindo inteiro no log (não-regressão). `go build`/`vet`/`staticcheck`/suíte do `internal/db` verdes. |
| ✅ | ~~**Atualizar era lembrar a URL do bundle — e ninguém tirava backup antes de trocar o binário**~~ | **Feito (2026-08-23, pedido do usuário: "qual esforço para empacotarmos no regente um comando de regente-update q faz o download install e restart?" + "fazer o backup mas da a opção de update sem backup").** O upgrade suportado já funcionava (reinstalar por cima: o `install-linux.sh` troca binário+UI, dá `daemon-reload`, **RESTARTA** e prova a troca pelo MainPID), mas o operador tinha de guardar a URL do release, e **nada tirava snapshot do banco antes** — migração de schema roda no boot seguinte e não tem desfazer. **`server/deploy/update.sh` → `/usr/local/bin/regente-update`** (mesmo padrão do `regente-configure`: viaja no bundle, instalado pelo `install-linux.sh`; entrou também no `cp` do `release.yml` e no espelho do `smoke-install.sh`). **Fluxo:** resolve a tag da `latest` **pelo redirect** (dá pra dizer "já está atualizado" sem baixar 40 MB) → compara com `regente-server -version` e sai sem tocar em nada se for igual (`-f` reinstala assim mesmo) → **backup online do banco** (`VACUUM INTO` no SQLite, `pg_dump -Fc` no Postgres; `--no-backup` pula, e Postgres sem `pg_dump` **aborta com instrução** em vez de atualizar às cegas) → guarda o binário anterior em `regente-server.bak` → delega pro `install-linux.sh` do bundle → imprime `vA -> vB`, o caminho do snapshot e a linha de rollback. **Dois detalhes que só aparecem em produção:** o `User=` vem da **unit instalada** (`systemctl show`), não do login de quem deu `sudo` — senão um update feito de outra conta reescreve a unit e o serviço perde o próprio `/var/lib/regente`; e o backup roda **como o usuário do serviço** (`runuser`/`su`), porque abrir o SQLite como root pode deixar `-wal`/`-shm` com dono root e quebrar a escrita **no boot seguinte**. `REGENTE_BUNDLE=<arquivo>` instala um bundle local (máquina sem internet — e é o gancho que o smoke usa). **Cobertura no portão de release** (`scripts/smoke/inside.sh`, stage2, num sistema com estado real depois do reboot): snapshot criado · binário anterior guardado · **processo trocado (39 → 211)** · health 200 · banco e workspace intactos · **no-op reconhece "already on" e NÃO reinicia** · `--no-backup` não cria snapshot. **10/10 verdes** (`smoke de instalação OK`). Fora do smoke fica só a perna do download (a release sob teste ainda não existe quando o smoke roda). Também: README (§Upgrading reescrito), `server/deploy/README.md`, `docs/site` regerado, e a string `[backup] snapshot online gravado em` — que era o último pt-BR na saída do binário — virou inglês (I18N). `go build`/`vet` verdes. |
| ✅ | ~~**Board vazio depois do login: o `_connected` ficava preso na fila de handshake do browser**~~ | **Feito (2026-08-23, reporte do usuário: "sempre q eu to na tela de login e entro ele não tem nada no canvas, aí eu tenho q apertar F5 pra aparecer").** O fix de 2026-07-01 ("board sem F5") atacou o sintoma certo pelo caminho errado: colocou TODA a recuperação pós-login em cima de um evento do WebSocket. **Medido ao vivo** (server dev com `-token` diferente do embutido no build, para o 401 pré-login ser real): `POST /api/auth/login` 200 → primeiro card no canvas **+3,2 s**, e **não veio do WS** — o socket só abriu 110 ms DEPOIS; quem encheu o board foi o retry de 5 s do store de instances, enquanto `defs`, folders, agentes, condições e alertas seguiam esperando o `_connected`. **Raiz 1 — rajada de handshakes condenados:** todo 401 chama `setAuthToken(null)` (handler de `unauthorized`), que chamava `reconnectNow()` **incondicionalmente**; na tela de login os polls (instances 5 s, resources 4 s) geraram **8 handshakes em 13 s**, todos rejeitados. Browser que vê handshake de WS falhando em série pro mesmo host passa a **ADIAR** o próximo (throttle por host, segundos → minutos) — e é nessa fila que o socket do login entra. Sem `_connected`, o único caminho de ressincronização não roda. **Raiz 2 — pós-login 100% depende do socket:** `_connected` era o gatilho ÚNICO dos seis pontos que buscam uma vez no mount (defs+publishedDefs · instances · folders/layouts · summary+sonda do cap da sidebar · pool de condições · alertas/daily/agents/env/me). **Fix:** (1) `setAuthToken` é **no-op quando o token não mudou de valor** — 401 não derruba mais o socket (2 tentativas em 24 s no mesmo teste, contra 8 em 13 s); (2) o login emite um evento **LOCAL** `_resync` (síncrono, sem socket no caminho) e os seis pontos passaram a assinar `isResyncEvent(ev)` = `_connected` ∨ `_resync`; (3) dedupe de 1 s: socket que abre logo após o login não repete a rodada inteira de GETs. **Validado ao vivo no mesmo cenário: login 200 → card no canvas em 38 ms** (de 3,2 s), sem F5 e sem rodada duplicada. `tsc`, `eslint` e `vite build` limpos. |
| ✅ | ~~**Shift na lista ACTIVE JOBS não selecionava a FAIXA**~~ | **Feito (2026-08-14, pedido do usuário: "clico no 0001, seguro o shift e clico no 0007 que é o último, aí seleciona tudo").** Shift e Ctrl faziam a mesma coisa (alternar 1 job), então montar uma seleção grande era um clique por job. Agora, **na lista**, Shift = faixa da âncora até a row clicada; **Ctrl** segue alternando e **Ctrl+Shift soma** a faixa à seleção. No **canvas nada mudou** — lá Shift continua sendo adicionar/alternar e a tecla do rubber-band do ReactFlow. A **âncora guarda posição E id**: a lista se remonta sozinha (poll de status, filtro, busca, carry-over entrando), então índice puro apontaria pra outro job — identidade que não bate mais degrada pra clique simples, e a faixa **nunca** inclui o que o operador não viu. Mesma razão pra faixa ser **dentro da folder**: cross-folder tem folder colapsada no meio e paginação independente. ⚠️ **No modo windowed** (dia acima do cap, rows por página sob demanda) uma faixa que atravessa página **nunca carregada** leva só o que está em memória — e isso **não é silencioso**: sai o toast `N of M rows selected`. Validado em rig de **600 jobs** (cap 500 → windowed): 0001→0024 na mesma página = 24; scroll longo pulando a página do meio e Shift no 0551 = **351 of 551** com toast, sem travar. Rows ganharam `userSelect:none` (senão o Shift do browser pinta seleção de texto por cima). Bulk continua com `window.confirm` contando os elegíveis, então faixa grande não vira ação em massa acidental. |
| ✅ | ~~**Seleção múltipla não aparecia na lista ACTIVE JOBS**~~ | **Feito (2026-08-14, pedido do usuário: "quando seleciono no canvas 2 jobs com control os 2 jobs ficam highlight, porém no menu lateral não").** Canvas e lista são duas superfícies da MESMA seleção (`selectedIds` do `V2Preview`, dono do highlight neon), mas a sidebar só recebia `selectedId` — o job do **drawer**, que é 1 só. Resultado: Ctrl+clique em 2 jobs acendia 2 cards e **1** row, nos dois sentidos (o clique já podia sair da própria lista, que manda `additive` desde a row-seleciona). `MonitoringSidebarV2` passou a receber **`selectedIds`** e a acender por `isSelected(id) = selectedIds.has(id) || selected === id`; o `||` mantém a row acesa quando a seleção foi limpa (pós-bulk) com o drawer ainda aberto, e é o único caminho no uso standalone do componente (sem a prop). O header ganhou o chip **`N sel`** (a partir de 2): a lista é **virtualizada**, então parte das rows acesas pode estar fora da viewport — ou fora do filtro/busca — e só o destaque não diria o tamanho da seleção. Validado no `regente dev` (4 jobs de demo) nos dois sentidos: Ctrl+clique no canvas → 2 rows com `--v2-accent-deep` + borda `--v2-accent-brand` + glow inset; Ctrl+clique na lista → 2 cards `.selected` no ReactFlow; Esc limpa a seleção e deixa acesa só a row do drawer. |
| ✅ | ~~**ACTIVE JOBS não encolhia: painel de 320px comendo o canvas o tempo todo**~~ | **Feito (2026-08-07, pedido do usuário: "fazer o active jobs ter uma opção de fazer ele se encolher para o lado ficando minimizado no canto com a opção de clicar e ele se expandir para o estado normal").** O painel já era arrastável e já persistia a largura, mas não tinha como sair da frente — num monitor pequeno os 320px (mínimo 240) ficavam permanentemente sobre o grafo. Agora colapsa para um **trilho de 44px** e volta com um clique. `useResizablePanel` ganhou **`railWidth`** (opcional; ausente = painel não colapsa, então `DesignSidebarV2` e `InstanceDetailsDrawer` seguem idênticos) e passou a devolver `collapsed`/`toggleCollapsed`, persistidos em **`<storageKey>.collapsed`**. A largura expandida fica **intacta** durante o colapso — o hook guarda `expandedWidth` separado do `width` devolvido, senão o `widthRef` do drag herdaria os 44px e reabrir jogaria o painel pro mínimo. O trilho **não é decorativo**: mostra o total do dia e os 4 status, e **clicar num status reabre o painel já filtrado** (`setFilter` + `toggleCollapsed`); os números saem do `fmtCompact`, que abrevia sempre **PRA BAIXO** (floor) porque inflar contagem visível seria mentir sobre o dia. Colapsado ele continua sendo o overlay **`data-canvas-inset="left"`**, só que com 44px — então a centralização do canvas (`useCanvasCamera.visibleInsets`, que mede o overlay por `getBoundingClientRect` na hora) segue honesta **sem ninguém precisar avisar a câmera**, e a câmera **não se move sozinha** ao colapsar (quem move a câmera é o usuário). ⚠️ **GOTCHA da virtualização, que era o único jeito real disso quebrar:** no trilho o scroller nem existe, e o efeito do `ResizeObserver` tinha deps `[]` — montar **já colapsado** (estado persistido) pegava a ref nula, retornava, e **nunca mais rodava**: ao reabrir, a lista virtualizaria pelo `viewH` default de 600px, sem observer, com a altura errada pra sempre. O efeito de medida **e** o de paginação (`ensureRange`, que sem isso buscaria páginas do server pra uma lista desmontada) passaram a depender do colapso. **Validado ao vivo** (`regente dev daily`, 4 jobs demo, SPA buildada com `VITE_REGENTE_SERVER_URL=@origin`) nos 4 caminhos: colapsar → inset medindo **44px** e `localStorage` em `"1"`; montar já colapsado + expandir → **scroller re-medido em 450px** (não os 600 do default) com as 4 rows renderizando; largura custom **520px preservada** no ciclo colapsa→expande; e clique no status **OK** do trilho reabrindo com o pill OK ativo. Zero erro de console. Sem hex novo — tudo em `--v2-*`, então os 17 temas vêm de graça. `tsc -b`, `npm run lint` e `npm run build` verdes. |
| ✅ | ~~**Statistics mentia a duração: job de 2 min com `avg 10min` / `max 98min` (ST-1)**~~ | **Feito (2026-08-05, auditoria pedida pelo usuário: "jobs que duram sempre 2 min têm avg 10~max 98m; isso provavelmente não tá certo" + "queria as últimas runs com início e fim, igual o orquestrador clássico").** **O diagnóstico:** a duração era `finished_at − started_at` da **instance** — e esse par não delimita uma execução. Quem o move não é só o job: (a) **Set OK num WAITING que já tentou** carimba `finished_at=AGORA` sobre um `started_at` de horas atrás (o retry agendado zera o `finished_at`, então o `COALESCE` do Set OK stampa o instante do clique) — o intervalo que entrava na estatística era *espera do operador*, não execução; (b) **retry e cyclic** sobrescrevem os timestamps a cada tentativa/volta, então a amostra só enxerga a ÚLTIMA e as anteriores somem; (c) **Set OK em quem nunca rodou** contava como "run" (e na taxa de sucesso) sem ter executado nada. O número denuncia a origem: com o **watchdog de RUNNING em 15 min** (`stuckRunningTimeout`), nenhuma execução real chega perto de 98 minutos — o `max` só podia vir de fora da execução. **A correção é estrutural, não um clamp:** a execução virou registro próprio, **`instance_runs` (schemaV23)** — uma linha por execução REAL, aberta no **claim `WAITING→RUNNING`** (a mesma transição que carimba `started_at`; é literalmente a régua que o usuário pediu: "início só se entrar em running, se ficar em wait não conta") e fechada no fim da **tentativa** (`FinishInstance`, ANTES do `maybeRetry` — cada tentativa é uma execução —, e `finishKilled` no cancel de um RUNNING). Claim revertido por falta de agente **apaga** a linha, junto com o `started_at` que o mesmo caminho já zerava. Ação de operador sobre quem não está rodando não abre nem fecha execução. `JobStats`, `PerfForecast` (p50/p90/próxima) e o MTTR do analytics passaram a medir sobre essa tabela; o `archiveGC` leva as execuções junto com a daily arquivada (senão órfão eterno, mesmo tratamento do `instance_output`), e a migration faz **backfill** (1 execução por instance que chegou a rodar) pra aba não nascer vazia numa instalação existente. **UI:** a aba Statistics ganhou **LAST RUNS** — as 10 últimas execuções com início, fim, tempo de execução e status (exit code no NOTOK, marca de tentativa quando é retry) —, o "This run" passou a mostrar a duração da EXECUÇÃO, e o **card do Monitoring lê "início–fim"** quando a run terminou (só o início enquanto roda). Pra caixinha não ficar com meia verdade até o próximo GET, o `finishedAt`/`startedAt` viajam **no próprio evento** `instance.changed` (o patch parcial da store aplicava só o status; o fim só aparecia num refresh cheio ou num clique — verificado ao vivo antes e depois), e sair de terminal APAGA o fim antigo, senão um rerun desenharia intervalo invertido. **Testes:** `runs_test.go` com 6 casos — ponta-a-ponta pelo `startInstance` real, 1 linha por tentativa no retry, kill fechando a execução, claim revertido sem execução aberta, "nunca rodou não é run" e a **regressão do caso auditado** (a instance fica com delta de 98 min e a Statistics reporta os 2 min da execução) — mais `Recent`/ordem/teto de 10 em `jobstats_test.go`. **Validado ao vivo** (`regente dev` demo-mode, 5 execuções na mesma ordem incluindo um NOTOK exit 137 via inject-failure): tabela renderizando as 5 linhas com início/fim/duração, `avg`/`max` presos em 1.0s, e o ciclo `WAIT AGENT → RUNNING│21:40 → OK│21:40–21:40` aparecendo na hora no card. `go vet`, staticcheck, suíte Go inteira, `tsc`, `npm run lint` e `npm run build` verdes; `GOOS=linux go build` idem. |
...
| ✅ | ~~**`middleware.RealIP` fora: IP do audit deixa de ser forjável por header**~~ | **Feito (2026-07-30, manutenção — piso).** O lote mensal do Dependabot (PR #4, chi 5.1.0→5.3.1) deixou a CI vermelha não por bump quebrado, mas porque o chi **deprecou `middleware.RealIP` citando GHSA-3fxj-6jh8-hvhx · GHSA-rjr7-jggh-pgcp · GHSA-9g5q-2w5x-hmxf** — e o staticcheck (gate) barrou o SA1019. O middleware confiava em `X-Forwarded-For`/`X-Real-IP`/`True-Client-IP` viessem de ONDE viessem, e pegava a entrada **mais à esquerda** do XFF, que é justamente a que o cliente escreve. Como `clientIP()` lia o `RemoteAddr` que ele reescrevia, **qualquer um forjava a origem das tentativas de login no audit/SIEM** mandando um header. Dimensionado ANTES de consertar: `RemoteAddr` não é consumido por autorização nem por rate limit (grep — o único consumidor é o `clientIP`), então era **corrupção da trilha de auditoria, não escalada de privilégio**. Substituído por `realip.go`: header só vale se o **peer TCP for proxy confiável** (`-trusted-proxies` / `REGENTE_TRUSTED_PROXIES`, default **loopback**, que é a topologia do `deploy/vps` — nginx em 127.0.0.1), varredura do XFF da **direita pra esquerda** parando no primeiro salto não-confiável, e CIDR inválido **derruba o boot** em vez de virar default silencioso. O IP resolvido vai pro **contexto**: o `RemoteAddr` para de mentir, que era o que tornava o bug invisível. 5 testes cobrindo o caminho de ataque (header de peer não-confiável ignorado nos dois headers, XFF direita→esquerda, fallback no peer, CIDR inválido). Verificado com o chi **5.3.1 do PR #4** aplicado localmente: `staticcheck` limpo e suíte do server inteira verde → o PR destrava com um rebase. |
| ✅ | ~~**Dependabot: alertas de segurança estavam DESLIGADOS**~~ | **Feito (2026-07-30, manutenção — piso).** O §🧰 Modo manutenção afirma que "patch de segurança chega fora do calendário pelo *Dependabot security updates*" — e esse caminho **não existia**: `GET repos/Dr0nj/regente/vulnerability-alerts` respondia **404** (204 = ligado) e a API de alerts dizia `Dependabot alerts are disabled for this repository`. O `dependabot.yml` commitado em `397e097` cobre só **version updates** (o lote mensal); alerta de segurança é outro switch. Sem ele, o único CVE visível era o que calhasse de vir no lote do mês — que é literalmente como o GHSA do chi (linha acima) apareceu: por acidente. Ligados os dois: `vulnerability-alerts` (agora 204) e `automated-security-fixes` (`{"enabled":true,"paused":false}`). 0 alerta aberto na varredura inicial. |
| ✅ | ~~**Selo de condição da edge acompanha o tema**~~ | **Feito (2026-07-29, reporte do usuário: nos temas claros os marcadores das dependências viram "borrões escuros" sobre o canvas).** Os selos ✓/✗ desenhados em cima das linhas tracejadas do canvas do Monitoring nasciam de hex CHUMBADO em `edgeStyleForState` (`#052e19` verde-escuro, `#450a0a` vermelho-escuro, `#1c1917` neutro) — calibrados pro fundo preto, viravam quadrado quase preto nos 4 temas claros. Agora saem de `--v2-edge-{ok,blocked,idle}[-label\|-bg]`, DERIVADOS por `color-mix()` dos `--v2-status-*` + fundo: tema novo herda o selo certo de graça, sem repetir 3 hex tema a tema (seriam 51 linhas). Os claros reabrem SÓ esses tokens num bloco único — diluir status num canvas claro não dá contraste pra um glifo de 9px, então lá o selo INVERTE (tint pálido do status + glifo escurecido). É o mesmo padrão dos `--color-*`: tema claro tem de sobrescrever os dois sistemas. **GOTCHA que quase virou um bug fantasma:** `fill` de SVG resolve `var()`/`color-mix()` normalmente, mas `getComputedStyle().fill` devolve `color(srgb 0..1)` e não `rgb(0..255)` — o primeiro probe assumiu `rgb`, leu `#000000` em todo tema e "provou" que o `color-mix` não tinha pegado. Validado ao vivo com `regente dev daily` (workspace demo: extract→transform→publish ligados por condição, demo-mode fecha OK → edges verdes): tokens lidos do DOM nos **17 temas**, contraste glifo/selo **4,4–11,6:1** (claros 7,7–8,8:1). `.react-flow__edge-path` também saiu do `#262626` chumbado. `tsc` e `npm run lint` limpos. |
| ✅ | ~~**Versão do build no rodapé da UI (VL-10)**~~ | **Feito (2026-07-28, pedido do usuário "coloca uma versão no Regente na UI, refletindo o release, pra gente saber se realmente atualizou, de preferência no rodapé").** Fecha o par com o VL-9: o instalador prova a troca **no momento** do upgrade; isto prova **depois**, a qualquer hora, sem SSH na caixa. `var version` no `main.go` injetada por `-ldflags "-X main.version=<tag>"` no `release.yml` (build do código-fonte fica **`dev`** — honesto, não há release por trás). Três superfícies: log de boot, **`regente-server -version`** (responde ANTES de abrir banco/workspace/porta, então roda numa máquina com o serviço já de pé) e **`GET /api/version`**, que é o que o rodapé lê. O endpoint fica **atrás da autenticação** de propósito: entregar a versão exata a um anônimo é entregar a lista de CVEs aplicáveis junto, e o `/health` público não precisa disso. **GOTCHA que só o browser pegou:** buscar no mount devolvia vazio — o `V2Preview` monta com a tela de login por cima e todo `/api` responde 401 a quem ainda não entrou; o efeito passou a depender de um **booleano derivado** do estado de auth (`me` troca de identidade a cada refetch e re-dispararia o efeito à toa). Validado ao vivo (server local com `-X main.version=v9.9.9-local` → rodapé `build v9.9.9-local`, lido do DOM) e no smoke, que builda com `smoke-test` e prova a cadeia inteira: ldflags → binário → `/api/version` → `-version` batendo com a API. `tsc` e `npm run lint` limpos. |
| ✅ | ~~**Reinstalar por cima não atualizava nada (VL-9)**~~ | **Feito (2026-07-28, pergunta do usuário "só preciso usar esse comando pra atualizar o meu servidor publicado?").** A resposta honesta era **não** — e o furo era do produto: o installer terminava em `systemctl enable --now`, e o `start` de uma unit **já ativa é no-op**. O binário novo ia pro disco e o **processo antigo continuava rodando**: reinstalava, tudo dizia "ok", nada mudava. Escapou porque o smoke sempre instalava em máquina limpa, nunca **por cima**. Agora o installer detecta o serviço vivo antes de tocar em qualquer coisa (guarda o `MainPID`), **restarta**, e PROVA no output: `UPGRADED in place` + `Process replaced: PID a -> b`; se o PID não mudar, avisa e manda restartar. Saída de upgrade passou a ser um resumo do que foi **preservado × substituído** em vez dos "próximos passos" de instalação nova. README com a seção **Upgrading a running installation** (tabela mantido × trocado, migrações rodam no boot, agentes são binários separados que se atualizam sozinhos, e zero-downtime só existe com N nós + Postgres — no single-box a resposta honesta é um restart de segundos). Smoke reinstala por cima e trava as 6 asserções (upgrade reconhecido · PID trocado · health 200 · `server.env` intacto · banco preservado · `deploy/vps` ainda instalado) — verde, PID 299 → 588. |
...
| ✅ | ~~**Instalação validada num VPS de mentira (container systemd) contra a release publicada — e 2 bugs de recuperação corrigidos (VA-9)**~~ | **Feito (2026-07-27, pedido do usuário "consegue seguir e testar em docker?").** Container `jrei/systemd-ubuntu:22.04` (`--privileged --cgroupns=host`, systemd de verdade, porta publicada) fazendo de VPS: rodei **o comando do README**, baixando a **release v0.2.0 do GitHub**, sem preparar nada. **Funcionou:** bundle baixado, UI single-origin instalada, unit `enabled`+`active`, `Health: ok` impresso pelo próprio installer, próximos passos com o IP real. Modo **offline por default** confirmado no log (`gitops=off`) — nenhum vestígio do repo do lab — e o aviso de `REGENTE_TOKEN=change-me` apareceu. `regente-configure` rodado num **pty** (`script -qec`): rejeitou uma URL colada e re-perguntou, aceitou `owner/name`, gerou token forte. **A validação achou o que a suíte não pegava:** com o repo errado, o serviço fica **`active`, `NRestarts=0`** e o retry roda em background (o binário da **v0.1.0**, mesma config, sai com **exit 1** — o crash-loop que o VA-5 matou) — **mas** o clone que falha no TRANSPORTE (HTTPS/credencial) deixa um **`.git` parcial** no workspace, e daí todo `EnsureClone` cai no ramo `IsRepo`: a instalação **nunca mais se recuperava**, nem com a config corrigida. Pior, nesse estado (zero commits, HEAD em `master`) o caminho "remote vazio → publica o workspace local" empurrava **nada** — **go-git não erra ao dar push de uma branch inexistente** — e o `fetch` seguinte reclamava `remote repository is empty` num loop infinito (foi exatamente o que apareceu no container: 3 retries iguais). **Correções:** falha de clone agora **limpa o workspace** (só se chega ali com ele vazio, então é seguro); com `.git` sem commit na branch alvo o caminho é o **bootstrap** (init+scaffold+commit+push), não o push vazio; e `pushLocked` **falha explicitamente** quando não há ref local para enviar, em vez de fingir sucesso. **Provado no container**: instalei o binário corrigido por cima (upgrade in-place) e o estado quebrado **se recuperou sozinho no primeiro boot** (`remote is empty and the local clone has no commit on main — bootstrapping` → `ready: main@c3dd47b`), com `.gitignore`/`README.md`/`definitions/.gitkeep` publicados no repo. **Resto do circuito validado**: `docker restart` (= reboot da máquina) → serviço sobe sozinho, workspace clonado, DB (286 KB) e UI intactos, `/api/git/status` sem erro; `install-agent.sh` da release recebendo **`http://127.0.0.1:8080`** (o endereço da UI, o erro típico) gravou `ws://127.0.0.1:8080/ws/agent`, token em `/etc/regente/agent.env` **0600** e zero token no `ExecStart`, agente **online** na frota; e o circuito completo **jobs-as-code**: YAML commitado no repo de workspace → `git/sync` → `Order Force` → **o agente executou de verdade** (status **OK**, `exitCode 0`, output `instalado pelo release e rodando de verdade`). Ainda: `regente-configure` morria no EOF do stdin (`set -e`) depois de gravar a config e antes de avisar do restart — `read … \|\| true` + aviso de restart sempre impresso. **2 testes novos** (`TestEnsureClone_FailedClone_DoesNotPoisonTheWorkspace`, `TestEnsureClone_HalfInitializedRepo_EmptyRemote_Bootstraps` — este VERMELHO no código anterior). Gates verdes. **Nota honesta:** o bare local criado por `git init --bare` nasce com HEAD em `master` e o server publica `main`, então um `git clone` dele avisa "remote HEAD refers to nonexistent ref" — no GitHub isso não acontece (push numa base vazia ajusta o default branch); é limitação do protocolo, não dá pra mexer no HEAD remoto por push. |
| ✅ | ~~**Instalar sendo leigo: release quebrada, install sem UI, workspace do lab como default e boot fatal (VA-1..VA-8)**~~ | **Feito (2026-07-27, pedido do usuário "avalia o Regente e o README principal e me diz se com as info que estão lá eu consigo instalar de forma fácil sendo leigo em um servidor" → "quero que você acerte todos os problemas e aplique as melhorias").** A auditoria testou os comandos do README de verdade e o veredito foi **não**. **(1) A release era o bloqueio real:** a última era a **v0.1.0 (267 commits atrás)** e trazia 6 assets — `install.sh`, `install-agent.sh`, `install-agent-windows.ps1` e os bundles `regente-server_linux_<arch>.tar.gz` **davam 404**, ou seja, TODO o caminho "from a release (no toolchain needed)" (opções 1 e 2 + agentes Linux/macOS/Windows) estava morto. Causa provável: os **5 jobs da matriz chamavam `softprops/action-gh-release` em PARALELO contra a MESMA release**. `release.yml` reestruturado — cada alvo sobe `artifacts`, e um job **`publish` único** baixa tudo (`merge-multiple`), anexa os scripts, **verifica a lista completa de 15 arquivos e FALHA antes de publicar** se faltar algum, gera `SHA256SUMS` e publica de uma vez; `workflow_dispatch` com input `tag` permite republicar uma tag existente. Cross-compile dos 5 alvos (server+agent) validado localmente. **(2) Do código-fonte, a opção "server + UI" instalava API-only em silêncio:** `install-linux.sh` procurava a SPA só em `$HERE/../app/dist` — que é o layout do BUNDLE (`<bundle>/app/dist`); no checkout o app está na RAIZ (`$HERE/../../app/dist`), então o script avisava e seguia sem UI. Agora procura nos dois layouts, aceita `SPA_DIR` explícito e o aviso ensina as duas saídas. **(3) O workspace do LAB era default de fábrica:** `-git-source=https://github.com/Dr0nj/regente-workspace.git` e `-github-repo=Dr0nj/regente-workspace` estavam **compilados no binário** (`main.go`), além de `server.env.example`, `configure.sh` e `host-demo.ps1` — e esse repo responde **404** pra qualquer outra pessoa. Como **o repo é diferente em cada instalação**, o default virou **vazio = modo offline**; `regente-configure` pergunta, valida `owner/name` (rejeita URL colada) e aceita vazio, comentando as chaves; `install-windows.ps1` e a demo idem. **(4) Repo vazio, o caso NORMAL de instalação nova, era erro:** `EnsureClone` agora trata `transport.ErrEmptyRemoteRepository` fazendo o **bootstrap** — `PlainInit` + remote + HEAD na branch pedida + scaffold (`definitions/.gitkeep`, `.gitignore` com os artefatos SQLite, `README.md`) + commit + push —, **adota** um workspace que já tem conteúdo quando o remote está vazio (migração offline→GitOps sem perder nada) e, quando a branch pedida não existe no remote, **segue a default do repositório** avisando (repo com `master`), em vez de falhar com "reference not found". A proteção contra clobber continua valendo quando o remote TEM conteúdo. **(5) Boot fatal virava crash-loop:** `log.Fatalf("[git] EnsureClone failed")` + `Restart=always` = serviço reiniciando pra sempre e **operador sem UI justamente onde arrumaria a config**. Agora o server sobe, loga o motivo, expõe `error` no `/api/git/status` (campo novo, também no tipo `GitStatus` do SPA) e no **badge git** (estado `⚠ not connected` com o motivo + o que fazer no tooltip), e uma goroutine tenta de novo com backoff 15s→5min; a daily é pulada até conectar (comportamento que já existia). `-github-repo` malformado também deixou de ser fatal. **(6) Bug que só a validação AO VIVO pegou:** com o clone falhando, o próprio server cria `<workspace>/definitions` logo depois — e o guard "not a git repo and not empty" passava a barrar **todas** as tentativas seguintes, então o retry **nunca** recuperava (só `rm -rf` manual). `IsEmpty` passou a ignorar diretórios vazios (`WalkDir` procurando o primeiro ARQUIVO). **(7) UX dos instaladores:** `install-linux.sh` faz health check em `127.0.0.1:<porta>/health` e imprime os próximos passos **na ordem** — `regente-configure` → restart → **abrir a porta no firewall** (ufw/firewalld + o lembrete do security group da nuvem, que faltava por completo) → URL com o IP real; os instaladores de agente **normalizam a URL** (`http://host:8080` → `ws://host:8080/ws/agent`, o erro mais comum de quem cola o endereço da UI), passaram a guardar o token em **`EnvironmentFile` 0600 / `EnvironmentVariables` do plist 0600** em vez da linha de comando (era legível por qualquer usuário via `ps`) e conferem se o serviço ficou de pé, imprimindo o log se não; `configure.sh` consulta o `/api/git/status` depois do restart e diz se o GitOps conectou. **(8) README:** seção "Before you start" (máquina, porta nos DOIS firewalls, criar o repo vazio + PAT com o escopo exato), o modelo de workspace **por instalação** e a persistência em `/var/lib/regente/workspace`, "Your first job" em 4 passos, tabela de troubleshooting e a correção de que só `server/deploy` e `agent/deploy` têm `install-windows.ps1` (o `deploy/` da raiz não tem). Ainda: aviso de segurança no boot quando `REGENTE_TOKEN` continua `change-me`/`dev-token`, e uma string PT que sobrou na UI (`nenhum job com esse filtro`) traduzida. **Testes:** 6 novos em `gitops_bootstrap_test.go` (bootstrap de remote vazio + idempotência, adoção do workspace, recusa com remote populado, **recuperação depois do boot falho**, fallback de branch, erro registrado no status). **Validado AO VIVO:** server real com repo inexistente → **sobe**, `/api/git/status` traz `error: git clone: authentication required: Repository not found.`, daily pulada, badge `⚠ not connected` na UI (verificado no browser); server real contra um bare VAZIO → bootstrap + push (`.gitignore`, `README.md`, `definitions/.gitkeep` no remote) → `daily: synced workspace to 0cf48cf`. Gates verdes: `scripts/verify.sh`, staticcheck (server+agent), `tsc`+`npm run lint`, docs/site regenerado. |
| ✅ | ~~**Documentação pública em inglês + site estático com a referência de API (I18N-6/7)**~~ | **Feito (2026-07-27, pedido do usuário "docsite, o swagger, os readmes... e o readme principal tem que ser auditado; e os docs precisam ser encapsulados de forma que alguém clique no readme e abra sem subir os servidores").** O produto já falava inglês desde a I18N-1..5, mas a DOC não. **Auditoria, não só tradução:** o README raiz foi reescrito em inglês simples para quem nunca usou um orquestrador (conceito em 4 frases, "try it in 5 minutes" com `regente dev`, capacidades reagrupadas por tema) e passou a cobrir TODAS as formas de instalar que existem de fato — server headless (`WITH_UI=0`), server+UI single-origin, server+agente local, agentes remotos (Linux/macOS/Windows, release E código-fonte), VPS com nginx+TLS, Docker/Knative, demo com túnel e o `importctm`. Os READMEs de componente estavam ERRADOS além de em português: o do server dizia Go 1.23, só SQLite e caminhos `projects/Regente/`; o do agent listava 3 executores dos 14. Traduzidos também `deploy/`, `deploy/vps/`, `deploy/demo/`, `server/deploy/`, `cmd/regente` e `cmd/importctm`, mais os docs publicados (`conditions-events`, `dr-backup`, `slos`, `mcp`) com dois RENOMES — `operacao.md`→`operations.md` e `arquitetura-futuro.md`→`architecture-future.md` — e todas as referências reescritas em código, docs e workflows. Corrigida de passagem uma contradição no `conditions-events`: o rótulo da aresta OR no canvas é **"OR"**, não "OU" (o doc dizia os dois). **Este roadmap fica em pt-BR e FORA do site** (decisão do usuário: "pode deixar o roadmap como doc nossa") — `skipDoc` no docsite exclui também linkedin-post, case-study PT (a `.en` é publicada) e os planos internos. **Empacotamento:** `docs/site/` saiu do `.gitignore` e virou **build VERSIONADO** (clonar/baixar e abrir o `index.html` basta — páginas self-contained, CSS inline), com passo novo no CI (`docs site up to date`) que regenera e FALHA se o commitado defasar; o docsite passou a emitir a **referência de API** (`api.html`) = o MESMO viewer do `/api-docs` (`api.OpenAPIAssets()`) com a spec **INJETADA** num `<script type=application/json>` — `fetch` é bloqueado em `file://` e num static host não há API atrás da página — e o viewer detecta o node injetado pra esconder Try-it e campo de token; `openapi.yaml`/`.json` vão junto. **Publicado no GitHub Pages** (`.github/workflows/pages.yml`, repo tornado público pelo usuário): **https://dr0nj.github.io/regente/** e `/api.html`, republicado a cada push na main que toque doc. **GOTCHA que só a validação AO VIVO pegou:** chamar `render()` síncrono no modo estático estourava `ReferenceError: Cannot access 'opSeq' before initialization` (temporal dead zone — o `render()` usa bindings `let` declarados mais abaixo no mesmo `<script>`; o caminho do fetch nunca viu porque roda async) → `queueMicrotask`. **O teste Go NÃO pega isso** (valida HTML/JSON, não execução): verificado abrindo o `file://` no browser e contando os cards — 28 endpoints, 0 Try-it, link de volta OK; depois reconferido na URL pública. 4 testes novos no docsite (site completo, skip de doc interno, referência de API estática, repo vazio). CI verde (server+agent build/vet/**staticcheck**/test · app build+lint · docs site up to date). |
| ✅ | ~~CI vermelho: flake `TempDir RemoveAll ... directory not empty` no `TestAction_RunJob`~~ | **Feito (2026-07-27):** o job `server` do CI falhou não-deterministicamente no run 30138959766 (commit `d32d1ab`, **docs-only** — não era regressão; passou no re-run) com `TempDir RemoveAll cleanup: unlinkat /tmp/TestAction_RunJob.../001: directory not empty` + `[scheduler] emitEvent cleanup-FORCE-.../started: sql: database is closed`. **Recorrência do tempdir flake (27ad8b9, 2026-07-04) por uma porta que sobrou.** Raiz: o `ForceOrder` despachava com **`go s.startInstance(id, *def)` SOLTA** — o rastreio existia só DENTRO do `startInstance` (`wg.Add(1)` na hora de montar o dispatch), e **`Add` depois do `go` é tarde demais**: o `wg.Wait()` do `Stop()` vê o contador em zero e retorna antes da goroutine sequer começar. O claim (`UPDATE`→RUNNING), o evento `started` e — no DemoMode dos testes — o `submitted` + o mock-finish de 1s caíam então depois do `db.Close()`, com o journal do SQLite recriado no diretório que o `RemoveAll` estava apagando. Caminho exercitado pelo teste: ação On/Do **`run-job` → `ForceOrder` → dispatch**. Fix: helper **`spawnTracked(what, fn)`** no scheduler — `wg.Add` no stack do **CHAMADOR** (antes do `go`, que é o que dá o happens-before com o `Wait`), `Done`+recover dentro, e **no-op se o `Stop()` já foi pedido** (`stopping()`: não spawna goroutine que só nasceria pra morrer no primeiro `sleepOrStop`; em produção o `Stop` só roda no shutdown, depois do `srv.Shutdown` — não abrir trabalho novo ali é o comportamento correto). `ForceOrder` e o backoff de retry passaram a usá-lo. **Dois testes novos** (`forcerun_test.go`), ambos VERMELHOS no código antigo: `TestForceOrder_StopWaitsForDispatch` (bus que trava no broadcast do RUNNING — a última chamada da parte síncrona ANTES do `wg.Add` interno, exatamente a janela do vazamento — e prova que o `Stop()` não retorna com o dispatch em voo) e `TestForceOrder_AfterStop_DoesNotDispatch` (depois do `Stop` a ordem forçada nasce WAITING e nenhum `started` é emitido). **Auditoria do resto:** todo teste do pacote `scheduler` nasce do `newTestScheduler` (o único `New()` de teste), que já registra `t.Cleanup(s.Stop)` — sem outro buraco lá; no pacote `api` a mesma classe aparecia **INVERTIDA**: `wow_test`/`differentials2_test` registravam `t.Cleanup(sched.Stop)` **antes** do cleanup que fecha o DB e `t.Cleanup` é LIFO, então o `Close` rodava primeiro e o `Stop` não protegia nada — e o `TestSelfService_GateByOptIn` chega no `ForceOrder` com DemoMode ligado; ordem corrigida nos dois + `sched.Stop()` no `query_test`. **Validação:** `-run TestAction_RunJob -count=20` verde, pacote `scheduler -count=3` verde, suíte do server `-count=1` verde, `GOOS=linux go vet ./...` e staticcheck limpos (`-race` não roda no dev Windows — sem gcc). |
| ✅ | ~~**Produto 100% em inglês — SPA, server, CLI, agente, contrato OpenAPI e scripts de instalação (I18N-1..5)**~~ | **Feito (2026-07-24, pedido do usuário "o Regente mistura inglês e português; deixa ele todo em inglês").** O produto falava as duas línguas ao mesmo tempo — aba "Condições" ao lado de "Schedule", erro de validação em pt-BR num contrato OpenAPI de título português, help do CLI misturado. **Regra adotada:** tudo que o **usuário** lê é inglês; tudo que o **desenvolvedor** lê (comentário de código, mensagem de commit, docs do repo) segue em pt-BR — a convenção do CLAUDE.md não muda. **I18N-1 (SPA, ~42 arquivos):** rótulos/abas/validações/toasts, os popovers do "?" (`HelpTopic`), `ScheduleEditor` (Mon–Sun, Jan–Dec, From/To, `toLocaleTimeString("en-GB")`), `theme.ts` (nomes/descrições — os **`id`** ficaram, são seletor CSS `data-theme` + chave de localStorage), `error-format.ts` e o **guia do dialeto YAML do modo Code** (`code-schema.ts`, ~650 linhas reescritas). **I18N-2 (server, ~57 arquivos):** validação/política que sobe pra UI, `typeschema.go` (summary + desc de todos os jobTypes), `explain.go` (blockers e summaries), `rca.go`, `bulk.go`, `conditions.go` (AND/OR + escopo `static`/`daily`), alertas do control plane (`selfmon.go`), `jobdsl`, `quickaction`, `oidc`, `sectls`, `db/backup`, SDK `pkg/client` e o catálogo MCP inteiro. **I18N-3 (CLI + agente):** help das flags, usage e saída de `regente`/`regente-server`; output dos jobs do agente (`[mft] OK: %d file(s)`, `[file-watch] waiting for…`, `timed out after %ds`, truncamento por maxRows); CLIs de apoio `importctm` (flags, relatório e as anotações `# TODO-import`), `docsite` e `seed`. **I18N-4 (contrato público):** `openapi.yaml` reescrito em inglês (info/tags/summaries/descriptions/schemas — o contrato é a cara da integração) + viewer `/api-docs` (`apidocs.html`: Parameters/Body/Responses/Try it/Send, `lang=en`). **I18N-5 (operador):** one-liner `install.sh`, installers de server e agente (Linux/macOS/Windows), `regente-configure` guiado, `backup`/`restore`/`rolling-upgrade`/`chaos-ha`, `enable-tls`, `sandbox-agent`, `host-demo` e `verify.sh` — só echo/prompt/erro. **NÃO traduzido de propósito:** termos canônicos do domínio (Schedule · On/Do · OK/NOTOK · ODAT · carry-over · Force Order · Run Now · Set OK · Hold/Release · WAIT_*), identificadores de máquina (ids de tema, motivos de carry, `GateKind`, nomes de jobType, tokens de condição) e os sinônimos PT do parser de NL-query (`api/query.go` — são tokens de **entrada**; a resposta já sai em inglês). **Método:** scanner Python (`scan_pt`) que remove comentários e casa literais de string + texto JSX contra lista de palavras PT + regex de acento, rodado como critério de conclusão — varredura final do app dá 11 hits, todos falso-positivo (`example.com`, `>=8`), e o Go só acusa `github.com`. **Testes que casavam com o texto PT foram atualizados** (query/output/mcp/database/awsjobs/filetransfer/conditionlogic/condlogic_gate/importctm/apidocs/pkg-client). **Gates verdes:** server e agent `go build`+`vet`+`test`+**staticcheck**; app `vite build` + `npm run lint`; `bash -n` nos 12 scripts alterados. 3 commits em `main` (`9b111c3`, `5f2b947`, `1344405`). |
| ✅ | ~~**On/Do ganhou o 4º gatilho: `on: exit` — dispara pelo EXIT CODE (lista/faixa/comparação), paridade enterprise clássica COMPSTAT**~~ | **Feito (2026-07-23, pedido do usuário "no On/Do tem 3 opções de quando; queria opções para status — quando o job finalizar com status 1, 2 ou 3 e o exit ser esse status").** O On/Do só sabia reagir ao status TERMINAL (`result`: OK\|NOTOK), à tentativa (`attempt`) e à duração (`runtime`) — mas o status é DERIVADO do código (`exit != 0` ⇒ NOTOK), então "meu job usa 1/2/3 como aviso e 8+ como erro grave" não tinha como ser expresso: todo código não-zero virava o mesmo NOTOK. **Gatilho novo `on: exit`** com o qualificador `exitCodes`, uma espec separada por vírgula (OR entre os itens) onde cada item é um **valor** (`3`, inclusive negativo — o KILL do Cancel grava `-1`), uma **faixa** (`1-4`, e `-3--1` p/ negativos) ou uma **comparação** (`>0`, `>=8`, `<0`, `<=2`, `!=0`); espec vazia/inválida é INERTE (nunca dispara "pra tudo", mesmo contrato de `Attempt==0`/`AfterMin==0`). Decisor puro novo `matchExitSpec` (`scheduler/actions.go`), avaliado no mesmo ponto terminal do `result` (`FinishInstance` + `finishKilled`, retries já esgotados) como dimensão INDEPENDENTE — as duas regras convivem na mesma def, cada uma com sua idempotência no ledger `action_fires`. As 4 ações valem: **`exit 1,2,3` + `set-ok` é o "trate estes códigos como sucesso"** (o que o usuário pediu), `>=8` + `notify critical` separa o erro grave do aviso. Sem migração de schema (campo novo no YAML/JSON com `omitempty`). **UI:** opção "Terminar com exit code X" no seletor Quando + campo Exit codes com hint da gramática e a tradução natural por regra ("Quando o job terminar com exit code 1, 2 ou 3, marca este job como OK") — `describeExitSpec` traduz faixa/comparação; guia do modo código com a tag `exitCodes` + exemplo; **Find & Update** ganhou o gatilho e — de quebra — os qualificadores que FALTAVAM lá (`attempt`/`afterMin`: regra em massa nascia inerte). **Testes:** `TestMatchExitSpec` (28 casos da gramática) + 4 casos novos no `TestActionMatches` + `TestAction_ExitCodeSetOK` (exit 2 na lista vira OK / exit 9 fora segue NOTOK) + `TestAction_ExitCodeNotify` (result e exit disparam juntos, idempotentes). **Validado AO VIVO E2E** (server real `-git-source` em bare repo + agente real na 18711, SPA na 15173): regra montada no drawer, YAML da session gravado com `on: exit`/`exitCodes: 1,2,3`/`do: set-ok`, modo código re-parseou ESTRITO (plano "~atualizar 1" — `KnownFields` aceita a tag nova), publish direto pra main, Force Order → agente executou `exit 2` → trilha de eventos `finished status=NOTOK exit=2` → `set-ok flipped from NOTOK to OK` → `action set-ok (auto-heal)`, instance final **OK**. Gates: `go build`/`vet`/**staticcheck**/`go test ./...` + `tsc --noEmit`/`npm run lint` verdes. |
| ✅ | ~~**Job novo nasce com retries 0 (era 2)**~~ | **Feito (2026-07-23, pedido do usuário "quero que o padrão seja 0 inicial").** Job criado no Design vinha com **2 re-tentativas** que ninguém pediu: uma falha virava 3 execuções silenciosas — inseguro pro tipo de job que NÃO é idempotente (o default seguro é rodar uma vez e parar; quem quer retry liga explicitamente). **3 portas fechadas:** (1) `v2/V2Preview.tsx` — o draft do drag&drop da paleta nascia `retries: 2` (a fonte de verdade do "job novo"); (2) `v2/JobConfigDrawer.tsx` (×2, useState + o reset do efeito) — o fallback `definition.retries ?? 2` **ressuscitava o 2** em job EXISTENTE salvo com 0: o Go serializa `retries` com `omitempty`, então retries=0 volta do server **sem o campo**, e o `?? 2` transformava "o operador escolheu 0" em 2 na próxima abertura do drawer (o `toWeb` do `ServerApiAdapter` já normalizava pra 0 — o drawer é que desfazia); (3) `lib/definition-store.ts` (`nodeToDefinition`, caminho legado) por consistência. Server intocado: `domain.JobDefinition.Retries` já é `int` zero-value. **Validado AO VIVO** (server real com `-git-source` em bare repo + SPA single-origin na 18521, design session de verdade): drawer NEW JOB abre com **Retries 0**; save gravou o YAML da session **sem linha `retries:`** (= 0, omitempty) com `timeout: 300`; reabrir o job (EDIT JOB, round-trip pelo server) mostra **Retries 0** — o 2 não volta. Zero erro de console; `tsc --noEmit` + `npm run lint` (gate CI) verdes. |
| ✅ | ~~**Bug: aba General do Monitoring mostrava "agent auto" em job PINADO (lia o pin no local errado)**~~ | **Feito (2026-07-23, report do usuário "criei os jobs com o agente Dron-teste mas a aba General mostra 'agent auto'; o log do job mostra o agente certo").** Não era impressão: o dado do server estava CORRETO — a instância traz `pinnedAgent:"Dron-teste"` (nível da instância) e `snapshotDef.agentId:"Dron-teste"` (snapshot da def), por isso o scheduler roteava certo e logava `agente 'Dron-teste' offline`. **Raiz (`v2/InstanceDetailsDrawer.tsx`, `GeneralTab`):** o cálculo do agente exibido lia o pin SÓ de `definition.actionConfig._agentId` — o local do pin no modelo de **Design** (def viva). Mas o Monitoring é IMUTÁVEL e alimenta o drawer com a instância mapeada do server, onde o pin mora em **`instance.pinnedAgent`** (mapeado em `server-instance-store.ts`, e no snapshot como `snapshotDef.agentId`), NÃO em `actionConfig._agentId`. Como o job estava WAITING (sem `output.agentId` de execução ainda) e `actionConfig._agentId` vinha vazio, caía no fallback **`"auto"`** — todo job pinado aparecia como "auto" na General mesmo com o pin correto em toda a stack. **Fix:** `pinnedAgent = instance.pinnedAgent || (definition.actionConfig._agentId ?? "")` — a fonte autoritativa do Monitoring (o snapshot congelado) vence, com o local de Design como fallback pra casos de def viva; ordem final `execAgent (executor real) → instance.pinnedAgent → actionConfig._agentId → "auto"`. **Validação:** `npm run lint` (gate CI) 0 erros, `vite build` OK, bundle novo (`InstanceDetailsDrawer-*.js`) contém `e.pinnedAgent||(...actionConfig?._agentId...)`; dado confirmado ao vivo via `GET /api/instances/{id}` (pinnedAgent + snapshotDef.agentId = "Dron-teste"). `JobInstance.pinnedAgent` já existia no `orchestrator-model.ts` (sem mudança de tipo). |
| ✅ | ~~**Job type PARALLEL REMOVIDO (mesmo caso de WAIT/CHOICE) — registry 15→14 tipos**~~ | **Feito (2026-07-22, pedido do usuário "PARALLEL tb é inútil, pode tirar").** Fecha a limpeza dos 3 "gate types" agentless: PARALLEL estava no MESMO limbo de WAIT/CHOICE — declarado `Agentless:true` no schema (`domain/typeschema.go`) mas SEM executor real (o único agentless de verdade é **SSH**, via `startInstance`→`runSSH`; o `executeJob` do agente não tem `case` para gate type nenhum), então nunca rodava. **Removido:** a entrada do registry + o `Check` cross-field de branches (`typeschema.go`, comentário do campo `Agentless` ajustado — não há mais gate interno); os casos de teste PARALLEL + o threshold do catálogo (`typeschema_test.go`, 15→14); a guia do modo código (`code-schema.ts`); a paleta do Design + o ícone órfão `Layers` (`DesignSidebarV2.tsx`); o seletor de tipo (`JobConfigDrawer.tsx`); o `case` do editor de params (`JobActionConfigEditor.tsx`); a união `JobType` + o `Record` `JOB_TYPES` + o import órfão `Layers` (`job-config.ts`); e as entradas dos mapas de exibição do drawer do Monitoring (`InstanceDetailsDrawer.tsx` — que ainda carregavam WAIT/CHOICE/PARALLEL por serem `Record<string,…>`, resíduo da 1ª leva). Tipo removido vira "desconhecido" (mundo aberto, params livres) — zero quebra p/ YAML legado. **Validação:** `go build ./...` + `go vet` + `go test` (domain/scheduler/api) verdes; app `tsc -b` + `vite build` + `npm run lint` (gate CI) verdes. |
| ✅ | ~~**Job types WAIT e CHOICE REMOVIDOS (redundantes) — registry 16→15 tipos**~~ | **Feito (2026-07-22, pedido do usuário).** Os dois eram tipos "agentless" declarados no schema (`domain/typeschema.go`) mas SEM executor real: o dispatch do scheduler só roda agentless para **SSH** (`startInstance`→`runSSH`), e o `executeJob` do agente não tem `case` para nenhum deles — na prática nunca rodavam (ficariam presos em WAIT_AGENT ou "unsupported jobType"). **Racional:** **WAIT** (delay/timer) é redundante — o `schedule` já tem hora marcada (`windowFrom`/`windowTo`) e, para uma pausa arbitrária, um `COMMAND` com `sleep` é mais simples que um tipo dedicado; **CHOICE** (desvio condicional) é coberto pelas **ações On/Do** (`run-job`/`set-condition`) + condições AND/OR na entrada. **Removido:** as duas entradas do registry + o `Check` cross-field de WAIT (`typeschema.go`); os casos de teste WAIT/CHOICE (`typeschema_test.go`); a guia do modo código (`code-schema.ts`); a paleta do Design (`DesignSidebarV2.tsx` — e os ícones órfãos `Clock`/`GitFork`); o seletor de tipo (`JobConfigDrawer.tsx`); os `case` do editor de params (`JobActionConfigEditor.tsx`); a união `JobType` + o `Record` `JOB_TYPES` + imports órfãos `GitBranch`/`Clock` (`job-config.ts`). **PARALLEL foi removido logo em seguida** (linha acima — mesmo caso). Tipo removido vira "desconhecido" (mundo aberto: aceito com params livres, mas só roda se um agente anunciar a capability) — sem quebra de validação para YAML legado. **Validação:** `go build ./...` + `go vet` + `go test ./internal/domain` verdes; app `tsc -b` + `vite build` + `npm run lint` (gate CI) verdes. |
| ✅ | ~~**Cancel = KILL de um job RUNNING (aborta o processo) + Set OK direto na espera**~~ | **Feito (2026-07-22, pedido do usuário).** O modelo de ações estava trocado: um job WAITING tinha botão **Cancel** (que não faz sentido — ele só está esperando) e não tinha como ser dado por OK direto; e um job RUNNING **não tinha Cancel** — o "Cancel" antigo só marcava CANCELLED no banco, sem tocar o processo que rodava no agente. Agora: **(1) Cancel virou KILL exclusivo de RUNNING** — o server manda um frame `{"event":"cancel"}` pelo MESMO canal de dispatch (roteado ao `agent_id` que roda a instance) nos 3 transportes (ws/http-long-poll/sse) **e no SERVER-AGENT embutido**; o agente rastreia as execuções por `instanceId` (`runRegistry`) e o cancel invoca a `context.CancelFunc`, que via **`exec.CommandContext`** derruba o processo de verdade (`runCommand`/`runScript`; o timeout também passou a viver no ctx). O job finaliza **NOTOK / exit -1** com output "(cancelled by operator — process killed)" **SEM auto-retry** (kill manual não é falha orgânica — decisão do usuário; mas **alertas e On-Do result=NOTOK disparam**). **(2) Guard idempotente no `FinishInstance`** (`isInstanceTerminal`): o resultado que o agente ainda mande depois do kill é IGNORADO — não ressuscita o estado nem re-arma o retry (também blinda o chaos que derruba um RUNNING). **(3) WAITING perdeu o Cancel e ganhou Set OK direto** (o `Scheduler.SetOK` já aceitava WAITING; era só a UI que escondia). Tudo nas **3 superfícies de ação** — drawer (`InstanceDetailsDrawer`), menu de contexto do canvas e barra de ação em massa (`BulkActionBar`: Cancel-all agora = RUNNING, Set-OK-all inclui WAITING). Testes: `scheduler/cancel_test.go` (kill→NOTOK sem retry + roteamento do frame + resultado tardio ignorado + WAITING→CANCELLED + terminal rejeitado) e `agent/cancel_test.go` (o cancel aborta um `sleep 30` em ~0.3s). |  sysout da execução sai da trilha de auditoria — tabela por tentativa + API live-tail + aba Output ao vivo + evento `wait` na timeline (paridade enterprise clássica)**~~ | **Feito (2026-07-22, pedido do usuário "executa a trilha OL").** O gatilho foi um teste AO VIVO: um job SCRIPT com ~5 min de stdout jogou TODO o sysout na aba **Logs** e deixou a aba **Output** vazia — porque cada chunk virava evento `kind=output` na MESMA tabela do jornal de agendamento (`instance_events`), que também é a trilha de auditoria (vazava pro feed `/api/events` cross-instance e pro export SIEM), e o `instances.output` só era gravado UMA vez no fim (nada ao vivo; retry perdia o sysout da tentativa falha). **OL-1 (armazenamento):** nova tabela `instance_output(id, instance_id, attempt, ts, chunk)` **schemaV22** — os chunks do agente (WS `output`/`POST /api/agent/output`) e do SSH server-side passam a APPENDar ali via `INSERT…SELECT COALESCE(attempts,1)` (resolve a tentativa corrente sem round-trip extra), gravados VERBATIM (o concat reproduz o stream); `attempt` separa cada tentativa → **retry deixa de perder o sysout da falha**; `FinishInstance` segue consolidando o texto final em `instances.output` (compat). Retenção PRÓPRIA `output_retention_days` (GC em lotes junto do auditGC) + CAP por instance `outputMaxBytes`=5 MiB (in-memory best-effort, UMA linha de aviso e descarta o resto; reabre no retry/finish); sysout morre junto no delete e no archiveGC (`scheduler/output.go`). **OL-2 (API):** `GET /api/instances/{id}/output?attempt=N` → `{attempts, attempt, text, complete, exitCode?}` — RUNNING serve o concat (live-tail), terminada o consolidado, com fallback pro `instances.output` em runs antigas sem chunks; `/instances/{id}/events`, `/api/events` e o export SIEM passam a EXCLUIR `kind=output` por default (`?include=output` mantém o legado legível — migração não destrutiva). **OL-3 (UI):** a aba Output virou LIVE-TAIL (`ServerOutputTab`) — poll 3s enquanto RUNNING, auto-scroll grudado no fim (solta ao rolar pra cima), **seletor de tentativa** quando `attempts>1` (mostra o sysout preservado das tentativas anteriores); mantém Command/Result; local mode intacto (`LocalOutputTab`). **OL-4 (timeline da espera):** `maybeEmitWait` no gate do tick grava evento `kind=wait` EDGE-TRIGGERED — só quando o motivo-bloqueador primário MUDA (dedup por instance em `waitReason`, limpo no `startInstance`): "aguardando condição X", "recurso em uso", "agente offline", "fora da janela", "aguardando Confirm" → o "por que demorou" vira HISTÓRIA consultável, não só o estado presente do Explain. **Testes:** `scheduler/output_test.go` (append por tentativa/retry, cap+aviso, retenção, wait edge-triggered) + `api/instance_output_test.go` (per-attempt, fallback consolidado, exclusão do legado no events/feed) + archiveGC cobre `instance_output`. **CI verde:** server build/vet/**staticcheck**/test · agent · app `tsc -b`+`vite build`+`npm run lint`. |
...
| ✅ | ~~**Refinamento UI: abas do drawer do Monitoring padronizadas com o Design + card violeta p/ "Wait for confirmation"**~~ *(rotulada "UI-3" no commit d04f0cd por engano — UI-3 do §Backlog é o override de grade por folder, entregue na linha acima)* | **Feito (2026-07-09, commit f77077e):** dois pedidos do usuário, só o VISUAL/UX (sem tocar ações/funcionamento). **(1) Padronização das abas** — as abas do `InstanceDetailsDrawer` (Monitoring) usavam pills com realce laranja (`.v2-tabrail`/`.v2-tab`/`.v2-tab-active`, o "círculo laranja"); passaram a usar o MESMO sublinhado fino do `JobConfigDrawer` (Design): inline `borderBottom:2px solid var(--v2-accent-brand)` na ativa, transparente nas outras, font mono, `borderRadius:0`, contador de deps vira ` (N)` inline. Abas e conteúdo idênticos; as classes `.v2-tab*` em `tokens.css` viraram CSS morto. **(2) Card "Confirm"** — job WAITING preso no gate clássico "Wait for confirmation" (`def.confirm && !confirmed`) agora fica **todo violeta** (`#a78bfa`: fundo `linear-gradient` composto sobre a surface + borda + glow) com **tag `✋CONFIRM`** e status lendo **"CONFIRM"**. Novo flag `JobNodeData.waitConfirm`, derivado em `canvas-layout.ts` (buildMonitoringCanvas) no MESMO padrão do `waitAgent` — lê a def viva de propósito, batendo com o gate real do server; **não fere a imutabilidade do card** (é bloqueio de execução, como waitAgent/waitEvent, não snapshot tipo dryRun). A seleção NEON continua vencendo o violeta. **Botão direito** no card ganhou **"Confirmar"** (primary, 1º item) quando o gate está ativo → `confirmInstance` (endpoint POST /instances/{id}/confirm, que já existia e é usado pelo botão do drawer); ao confirmar o card sai do violeta e o job executa. Plumbing já existia (`def.confirm`/`inst.confirmed` já chegavam no front via `ServerApiAdapter.toWeb`/`server-instance-store.toWeb`). `tsc` + `vite build` verdes; **validado AO VIVO** no preview (computed styles: aba ativa `2px solid accent`/radius 0/mono = Design; card com bg gradient violeta + borda `rgb(167,139,250)` + tag + menu "Confirmar"; e limpeza do violeta ao confirmar → job roda a OK). |
...
| ✅ | ~~Aprofundamento enterprise: Actions/On-Do (motor backend)~~ | **Feito (2026-06-29):** motor On/Do nas 3 dimensões (result/attempt/runtime) + 4 ações (notify/set-condition/run-job/set-ok) reusando substratos; idempotência por ledger (migration v7); 14 testes + validado ao vivo. |
| ✅ | ~~Aprofundamento enterprise: Actions/On-Do (UI por job)~~ | **Feito (2026-06-30):** aba "On/Do" no JobConfigDrawer (OnDoEditor) — regras On‹gatilho›Do‹ação› com add/remove, campos contextuais, chips de canais e tradução natural por regra. ActionRule no modelo TS + map no ServerApiAdapter; round-trip provado (TestFileStore_ActionsRoundTrip). |
...
| ✅ | ~~**Aprofundamento enterprise (restante)** — cyclic · CONFIRM · DATABASE · SET var + datas · baterias · ViewPoints~~ | **Feito (2026-07-03):** TRILHA 100%. Ver a linha do topo do changelog. |
...
### E6 — Importador de orquestrador legado (redutor de fricção nº 1 p/ adoção) ✅ *(entregue 2026-07-07 — ver changelog + README do cmd)*
**Contexto:** empresas que avaliarem o Regente têm CENTENAS de jobs no modelo clássico;
migrar na mão mata a adoção.
**Spec:** novo binário `server/cmd/importctm` (pure-Go, sem deps novas): lê o XML de
export do modelo clássico (`DEFTABLE`/`FOLDER`/`JOB` do ctm export/forecast) e gera um
...
## 🧩 Aprofundamento enterprise *(SPEC/histórico — status resumido no §✅ Entregue)*
...
✅ Ciclo de vida na daily (Keep Active / carry-over entre diárias) — ENTREGUE (2026-06-24):
   ...
   Mecanismo (New Day clássico): `scheduler.carryOver(date)` roda no topo do `RunDaily` (antes da
...
✅ Sistema de variáveis (estilo enterprise clássico %%) → COMPLETO. Interpolação (2026-07-02) + SET em runtime
...
- ✅ MFT (FILE_TRANSFER nativo) · Archives/Retention · Import de orquestrador legado · CLI/SDK · site de docs
...
> **Onde o Regente passa o orquestrador clássico**, organizado por tema — o *porquê* de cada diferencial.
> O **status e a implementação** de cada um estão no §✅ Entregue (seção "🌟 Diferenciais além do
> enterprise clássico", trilha fechada). Esta seção é a narrativa pra Fase Z / case study — não repete status.

1. **Orquestração híbrida e stateful** *(grande gap do modelo clássico)* — human-in-the-loop + long-running
...
3. **Developer experience** *(onde o orquestrador clássico perde feio)* — schedule-as-code (YAML + DSL), `regente test`
