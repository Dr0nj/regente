...
> ✅ **V1 + V2 ENTREGUES (2026-07-11)** — o install serve a UI single-origin e há bundle
> nas Releases + one-liner `curl … | sudo bash`. Detalhe no §Entregue "📦 Self-hosting em
> VPS" + changelog. Restam V3–V6.
...
✅ V2 · Binário de server nas Releases + one-liner de instalação
       ...
       delega pro install-linux.sh. Zero Go/Node no VPS.
─────────────────────────────────────────────────────────────────────────────
⬜ V3 · Bootstrap interativo de config     → §Backlog
⬜ V4 · HTTPS estável (ACME/Tunnel nomeado) → §Backlog
⬜ V5 · Demo pública em Linux (agente sandbox Docker) → §Backlog
