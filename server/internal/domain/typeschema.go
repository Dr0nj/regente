...
var jobTypeSchemas = []JobTypeSchema{
	...
	{
		Type: "BATCH", Aliases: []string{"AWS_BATCH"}, Summary: "Job em container/lote (AWS Batch) — SubmitJob + poll pelo agente (SigV4, sem SDK).",
		Fields: []FieldSpec{
			...
			{Name: "jobName", Kind: KindString, Desc: "nome da submissão (default regente-<unix>)"},
			{Name: "command", Kind: KindString, Desc: "override do command do container (split por espaços)"},
			{Name: "env", Kind: KindMap, Desc: "variáveis de ambiente extras"},
			{Name: "parameters", Kind: KindMap, Desc: "parameters da job definition"},
			{Name: "region", Kind: KindString, Desc: "default env AWS_REGION do agente"},
			{Name: "accessKeyId", Kind: KindString, Desc: "default env AWS_ACCESS_KEY_ID"},
			{Name: "secretAccessKey", Kind: KindString, Desc: "default env AWS_SECRET_ACCESS_KEY"},
			{Name: "sessionToken", Kind: KindString, Desc: "default env AWS_SESSION_TOKEN"},
	...
	{
		Type: "GLUE", Aliases: []string{"AWS_GLUE"}, Summary: "Job de ETL (AWS Glue) — StartJobRun + poll pelo agente (SigV4, sem SDK).",
		Fields: []FieldSpec{
	...
	{
		Type: "STEP_FUNCTION", Aliases: []string{"STEP_FUNCTIONS"}, Summary: "Dispara uma state machine (AWS Step Functions) — StartExecution + poll pelo agente (SigV4, sem SDK).",
		Fields: []FieldSpec{
			...
			{Name: "name", Kind: KindString, Desc: "nome da execution (default: AWS gera)"},
			{Name: "region", Kind: KindString, Desc: "default env AWS_REGION do agente"},
			{Name: "accessKeyId", Kind: KindString, Desc: "default env AWS_ACCESS_KEY_ID"},
			{Name: "secretAccessKey", Kind: KindString, Desc: "default env AWS_SECRET_ACCESS_KEY"},
			{Name: "sessionToken", Kind: KindString, Desc: "default env AWS_SESSION_TOKEN"},
