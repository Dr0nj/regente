...
func (s *Scheduler) instanceRetentionDays() int {
	...
	if err != nil || n < 0 {
		log.Printf("[scheduler] settings instance_retention_days %q inválido (esperado inteiro ≥ 0) — retenção desligada", v)
		return 0
	}
	return n
}
...
func (s *Scheduler) archiveDay(day string) error {
	...
	//  1. events das instances do dia (senão órfãos eternos);
	//  2. as instances;
